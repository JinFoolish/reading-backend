...
class Jwt(object):
    key = 'dududu.minipro.key'

    @staticmethod  # 静态方法的装饰器封装一下  专门负责做计算用的函数
    def encode(self_payload, exp=300):
        ...
        # init sign
        hm = hmac.new(Jwt.key.encode(), header_json_base64 + b'.' + self_payload_copy_json_base64,
    ...
    def decode(token):
        ...
        hm = hmac.new(Jwt.key.encode(), header_bs + b"." + payload_bs, digestmod="SHA256")
