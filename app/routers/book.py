...
router = APIRouter(
    prefix='/book', tags=['book'], dependencies=[Depends(get_token_authorize)])
...
async def list_book(cate: Union[str, None] = None, query: PageReq = Depends(PageReq)):
    ...
    data = []
    if query.q:
        search_q = search_q & (Q(title__contains=query.q)
                               | Q(author__contains=query.q))
    if cate:
...
async def batch_list(batch: List[str]):
    ...
    for book_id in batch:
        book_info = BookInfo.objects(id=ObjectId(
            book_id)).exclude('introduce').first()
...
@router.get('/delete')
async def delete_book(user_id: str, book_id: str):
    UserBook.objects(user_id=user_id, book_id=book_id).delete()
    return {}
